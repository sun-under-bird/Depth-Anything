...
def parse_args():
    """解析 metric 深度模型导出 ONNX 所需的命令行参数。"""
    parser = argparse.ArgumentParser(description="将 Depth Anything V2 metric 模型导出为 RKNN 可转换的 ONNX。")
    parser.add_argument("--encoder", default="vits", choices=MODEL_CONFIGS.keys(), help="模型编码器类型。")
    parser.add_argument("--checkpoint", required=True, help="metric depth 的 .pth 权重路径。")
...
def build_model(encoder, checkpoint, max_depth):
    """加载 metric 深度模型和权重。"""
...
def main():
    ...
    model = build_model(args.encoder, args.checkpoint, args.max_depth)
